...
fn lines(source: &[u8], tokens: &[Token]) -> Vec<u32> {
    // Marked lines are a subset of all lines, so the newline count bounds the
    // rows. One extra token pass buys every regrow back.
    let newlines = tokens
        .iter()
        .filter(|token| token.kind() == TokenKind::Newline)
        .count();
    let mut out = Vec::with_capacity((newlines + 1) * stride::LINES);
...
fn blocks(source: &[u8], tokens: &[Token], cst: &Cst) -> Vec<u32> {
    // One row per non-root node at most — an exact bound, and free.
    let mut out = Vec::with_capacity(cst.nodes.len() * stride::BLOCKS);
...
fn notes(source: &[u8], tokens: &[Token], cst: &Cst) -> (Vec<u32>, Vec<u32>) {
    // Notes are a small minority of nodes; a hint in the scanner's style
    // (`source.len() / 6`), not a bound — a wrong guess costs a doubling.
    let mut extents = Vec::with_capacity(cst.nodes.len() / 8 * stride::NOTE_EXTENTS);
    let mut parts = Vec::with_capacity(cst.nodes.len() / 8 * stride::NOTE_PARTS);
    for (node, row) in cst.nodes.iter().enumerate() {
        if row.token == ROOT_TOKEN {
            continue;
        }
