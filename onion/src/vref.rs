...
/// A sid renders as `BOOK C:V` — this many bytes covers the corpus without a
/// regrow, and a longer one costs one doubling, not a wrong answer.
const SID_HINT: usize = 12;

/// Total masked bytes, O(1) off the mask's own zip-mate index. The tight bound
/// on every text these three functions concatenate.
fn masked_len(mask: &Mask) -> usize {
    match (mask.starts.last(), mask.ranges.last()) {
        (Some(&start), Some(range)) => start as usize + range.len(),
        _ => 0,
    }
}
...
pub fn keys(toc: &Toc, mask: &Mask, source: &[u8]) -> String {
    use core::fmt::Write;
    let mut out = String::with_capacity(toc.verses.len() * (SID_HINT + 1));
    for (sid, _) in verses(toc, mask, source) {
        if !out.is_empty() {
            out.push('\n');
        }
        // Straight into the output: a sid's own `to_string` would be one
        // throwaway allocation per verse in the book.
        let _ = write!(out, "{sid}");
    }
    out
}
...
pub fn lines(toc: &Toc, mask: &Mask, source: &[u8], trim: bool) -> String {
    let mut out = String::with_capacity(masked_len(mask) + toc.verses.len());
    for (_, text) in verses(toc, mask, source) {
        if !out.is_empty() {
            out.push('\n');
        }
        // Trimming through the borrow, not through a second String.
        out.push_str(if trim { text.trim() } else { &text });
    }
    out
}
...
pub fn joined(toc: &Toc, mask: &Mask, source: &[u8], trim: bool) -> String {
    ...
    let mut out = String::with_capacity(
        masked_len(mask) + toc.verses.len() * (SID_HINT + JOIN.len_utf8() + 1),
    );
    for (sid, text) in verses(toc, mask, source) {
        if !out.is_empty() {
            out.push('\n');
        }
        let _ = write!(out, "{sid}{JOIN}");
        out.push_str(if trim { text.trim() } else { &text });
    }
    out
}
