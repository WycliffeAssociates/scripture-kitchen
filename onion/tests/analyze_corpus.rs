...
//! fix index resolves.
//!
//! Files: every `*.usfm` under `testData/exampleCorpora/` plus
//! `testData/usfmtc/samples-from-wild/hindi-IRV1/` (dense Devanagari, where byte and
//! UTF-16 offsets drift on nearly every character). Both trees are committed under testData/;
...
fn ref_verses(source: &[u8], tokens: &[Token], table: &Toc, u: &dyn Fn(u32) -> u32) -> Vec<u32> {
    ...
    for row in &table.verses {
        ...
        let shaped = usfm_onion::designator::verse(ref_designator_bytes(source, tokens, row.token))
            .range()
            .is_some();
        out.extend_from_slice(&[
...
fn corpus() -> Vec<PathBuf> {
    ...
    collect_usfm_paths(
        Path::new("../testData/usfmtc/samples-from-wild/hindi-IRV2"),
        &mut paths,
    );
    paths.sort();
    paths
...
fn a_hindi_verse_anchor_is_utf16_not_bytes() {
    let path = Path::new("../testData/usfmtc/samples-from-wild/hindi-IRV1/origin.usfm");
...
fn every_corpus_book_agrees_with_the_reference_emitter() {
    ...
    if paths.is_empty() {
        eprintln!(
            "analyze oracle SKIPPED: no *.usfm under testData/exampleCorpora/ or testData/usfmtc/"
        );
        return;
    }
