...
/// Every artifact here is regenerated by the same bin.
const FIX: &str = "cargo run --bin codegen";
...
fn generated_rs_is_not_stale() {
    ...
    assert!(
        fresh == CHECKED_IN,
        "{}",
        ticket::stale::first_difference("src/tables/generated.rs", FIX, &fresh, CHECKED_IN)
    );
}
...
fn diagnostics_json_is_not_stale() {
    ...
    assert!(
        fresh == CHECKED_IN_DIAGNOSTICS,
        "{}",
        ticket::stale::first_difference(
            "onion-wasm/diagnostics.json",
            FIX,
            &fresh,
            CHECKED_IN_DIAGNOSTICS
        )
    );
}
...
fn wire_writer_is_not_stale() {
    ...
    assert!(
        fresh == CHECKED_IN_WIRE,
        "{}",
        ticket::stale::first_difference("src/wire/generated.rs", FIX, &fresh, CHECKED_IN_WIRE)
    );
}
...
fn wire_reader_is_not_stale() {
    ...
    assert!(
        ...
        "{}",
        ticket::stale::first_difference("onion-wasm/reader.ts", FIX, &fresh, CHECKED_IN_READER)
    );
}
