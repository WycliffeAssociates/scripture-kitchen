...
- `ticket/` is the second zero-dependency leaf: the WIRE vocabulary (`Width`,
  ...
  galley's find buffer — so no two formats can disagree about how a field
  reaches the buffer or where a reader looks for it. A format's own
  declaration, template, magic, version, ENVELOPE, codegen bin and generated
  files stay with the format; `ticket` never knows a format exists. It is not
  in `mise` on purpose — `mise` excludes generators — and it is generator-only,
  so nothing on a runtime path calls it. Scope rule: `ticket/README.md`.
- Every wire format is CHECKED IN at both ends with a staleness test, and the
  ...
  `galley/find-reader.ts` (find), `galley/sous-reader.ts` (the findings
  snapshot, generated by `sous-core`'s own emitter). A consumer never learns a
  layout, so a version shift is a loud failure at `open` rather than a field
  silently misread. Two formats are still read by hand and are the standing
  debt: galley's corpus envelope (`ONCP`, written in `galley/src/corpus.rs`,
  decoded inside `onion-wasm/reader.ts.tmpl`) and sous-core's, whose reader is
  typed in TypeScript with only its numbers generated.
