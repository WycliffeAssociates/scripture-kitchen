...
| (edit-neighbors moved back to `core/examples/edit_neighbors.rs`, 2026-09-07) | typo candidates one edit from a frequent word | dropped as a CHANNEL (230 ms–1.9 s per corpus, noisy); Will: not a loser — a cold, manual "check for possible typos" action is its own part of sous if the filtered yield is real. Being re-measured with name/length/first-letter filters |
