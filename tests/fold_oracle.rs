...
//! the same law over every corpus book. The corpus is gitignored, so that
//! half skips loudly when `example-corpora/` is not mounted.
...
fn fold_oracle_over_every_corpus_book() {
    ...
    collect(Path::new("example-corpora"), &mut paths);
    if paths.is_empty() {
        eprintln!("fold oracle SKIPPED: no *.usfm under example-corpora/");
        return;
    }
    paths.sort();
    for path in &paths {
