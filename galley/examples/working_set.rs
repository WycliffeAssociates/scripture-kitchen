...
//! public API itself reports (`Pantry::resident_bytes`,
//! `Expediter::resident_bytes`, `Warmer::resident_bytes`) to see what the
//! host's own bookkeeping does and does not account for.
//!
...
fn main() {
    ...
    for step in 1..=200usize {
        ...
        if step == 50 || step == 100 || step == 200 {
            ...
            print_row(
                &checkpoint,
                &format!(
                    "resident_bytes={} avg allocs/(keystroke+publish) over [{prev_step},{step}] = {:.0}",
                    fmt_bytes(sous.resident_bytes()),
    ...
    // -------------------------------------------------------- the breakdown
    //
    // What each accessor gives us, and what it does not:
    //
    // - `Pantry::resident_bytes()` is warmer bytes PLUS the sum of every
    //   registered book's own products (mask + toc + utf16 table +
    //   fingerprint + retained text + struct overhead) — see
    //   `galley/src/pantry.rs`'s private `book_bytes`. There is no public
    //   accessor that returns any ONE of those components for a book living
    //   inside a resident `Expediter`: `Expediter::pantry()` returns `&Pantry`
    //   only, and `Pantry::book()` (which hands back an `Entry` with
    //   `.mask()`/`.toc()`/`.utf16()`) needs `&mut Pantry`. GAP: no
    //   `Expediter::pantry_mut()` and no `Pantry::text_bytes()` /
    //   `Pantry::product_bytes()` split. So "Pantry products" below is a
    //   RESIDUAL (resident_bytes() minus warmer bytes minus retained text
    //   bytes), not a direct per-component read.
    // - Retained text bytes: no accessor either; computed here from the
    //   harness's own record of what each book was last `update`d with,
    //   which is exactly what `Retain::Text` (the default) keeps a copy of.
    // - `Fingerprint::resident_bytes()` exists but is PRIVATE. GAP: from
    //   outside the crate the only public handle is `chunk_count()`, so a
    //   fingerprint's footprint can only be estimated as
    //   `chunk_count() * (4 + 16)` bytes (one `u32` start plus one 16-byte
    //   `RawChecksum` per chunk) — folded into the same "Pantry products"
    //   residual above rather than teased out further.
    // - `Warmer::resident_bytes()` and `Expediter::resident_bytes()` ARE
    //   direct, exact reads of the host's own ledgers.
    let text_bytes: usize = corpus
        .iter()
        .enumerate()
        .map(|(i, (_, original))| live_texts.get(&i).map_or(original.len(), |t| t.len()))
        .sum();
    ...
    println!(
        "{:<58} {:>12}",
        "Pantry products (mask+toc+utf16+fingerprint; residual, see gap above)",
