...
//! ```
//!
//! The bits are the general-category and grapheme-break lanes both engines
//! read: `sous_core::unicode` for its neighbour pools and atom rule,
//! `galley::find` for the whole-word boundary. Each remaining lane — script,
//! quote set, word break, normalization prefilter — returns only with a
//! consumer.
//!
//! `table.rs` is generated and committed, so nothing is read at runtime. The
//! generator needs a hash map and `mise` takes no dependencies, so it stays in
//! `sous-core` beside the pinned UCD extracts and writes here:
//! `cargo run -p sous-core --bin gen-unicode`. Bit table, index paths, and
//! gates: `sous-chef/core/src/unicode/README.md`.

pub mod lookup;
mod table;

/// One scalar's classification bits. Callers read them through the
/// predicates; the layout belongs to the generator.
...
impl Class {
    /// The inverse of [`bits`](Self::bits) — a const-context `Class(0)` and
    /// the generator's round trip.
    ...
    /// Glue is Mark plus grapheme extenders: what cannot stand alone.
...
/// [`Class::is_glue`], as a scalar predicate.
#[inline]
